...
public class StorageRepository {
    ...
    private StorageReference ref, answerRef;
    ...
    public StorageRepository() {
        ...
        this.answerRef = storageRef.child("answers/");
    }
    ...
    public Task<List<String>> uploadAnswerImages(ArrayList<Uri> imageList, int uploadCount, Context context, ImageAdapter imageAdapter, String name) {
        List<Task<UploadTask.TaskSnapshot>> tasks = new ArrayList<>();
        List<StorageReference> imageRefs = new ArrayList<>();
        TaskCompletionSource<List<String>> taskCompletionSource = new TaskCompletionSource<>();
        for (uploadCount = 0; uploadCount < imageList.size(); uploadCount++) {
            ...
            StorageReference imageName = answerRef.child(file);
            tasks.add(imageName.putFile(individualImage));
            imageRefs.add(imageName);
        }
        Tasks.whenAllSuccess(tasks).addOnSuccessListener(new OnSuccessListener<List<Object>>() {
